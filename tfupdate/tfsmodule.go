...
// tfsModuleSourceRegexp is a regular expression for module source.
...
var tfsModuleSourceRegexp = regexp.MustCompile(`(.+)\?ref=v([0-9]+(\.[0-9]+)*(-.*)*)`)

// ModuleUpdater is a updater implementation which updates the module version constraint.
type TfsModuleUpdater struct {
	name      string
	source string
	newSource string
...
// NewTfsModuleUpdater is a factory method which returns an ModuleUpdater instance.
func NewTfsModuleUpdater(name string, 	source string, newSource string, version string, nameRegex *regexp.Regexp) (Updater, error) {
	if len(name) == 0 {
...
func (u *TfsModuleUpdater) Update(_ context.Context, _ *ModuleContext, filename string, f *hclwrite.File) error {
...
func (u *TfsModuleUpdater) match(name string) bool {
	if u.nameRegex == nil {
		return u.name == name
	}
...
func (u *TfsModuleUpdater) updateModuleBlock(f *hclwrite.File) error {
	for _, m := range allMatchingBlocksByType(f.Body(), "module") {
		if s := m.Body().GetAttribute("source"); s != nil {
			name, version := tfsParseModuleSource(s)
...
// tfsParseModuleSource parses module source and returns module name and version.
func tfsParseModuleSource(a *hclwrite.Attribute) (string, string) {
	...
	if len(tokens) == 3 &&
		...
		matched := tfsModuleSourceRegexp.FindStringSubmatch(source)
		if len(matched) == 0 {
			// no version number
			return source, ""
		}
